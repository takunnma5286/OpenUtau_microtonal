    ...
    public class SingerSetupViewModel : ViewModelBase {
        ...
        private void RefreshTextItems() {
            ...
            using (var archive = ArchiveFactory.Open(ArchiveFilePath, readerOptions)) {
                try {
                    textItems.Clear();
                    foreach (var entry in archive.Entries.Where(entry => entry.Key!.EndsWith("character.txt", StringComparison.OrdinalIgnoreCase) || entry.Key.EndsWith("oto.ini", StringComparison.OrdinalIgnoreCase) || entry.Key.EndsWith("oto.txt", StringComparison.OrdinalIgnoreCase))) {
                        using (var stream = entry.OpenEntryStream()) {
