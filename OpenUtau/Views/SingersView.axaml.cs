...
namespace OpenUtau.App.Views {
    public partial class SingersView : UserControl, ICmdSubscriber {
        ...
        async void OnSetImage(object sender, RoutedEventArgs args) {
            ...
            var file = await FilePicker.OpenFile(
                TopLevel.GetTopLevel(this)!, "singers.setimage",
        ...
        async void OnSetPortrait(object sender, RoutedEventArgs args) {
            ...
            var file = await FilePicker.OpenFile(
                TopLevel.GetTopLevel(this)!, "singers.setportrait",
                viewModel.Singer.Location,
                FilePickerFileTypes.ImageAll);
            if (file == null) {
                return;
        ...
        async void OnPublish(object sender, RoutedEventArgs args) {
            if (OS.IsWasm()) {
                await MessageBox.Show(this, "Feature not supported on Web.", "Warning", MessageBox.MessageBoxButtons.Ok);
                return;
            }
            var viewModel = (DataContext as SingersViewModel)!;
            if (viewModel.Singer == null) {
        ...
        void OnSelectedSingerChanged(object sender, SelectionChangedEventArgs e) {
            var plot = this.FindControl<OtoPlot>("OtoPlot");
            if (plot != null) {
                plot.WaveFile = null;
            }
            var playBack = PlaybackManager.Inst.AudioOutput;
        ...
        void GotoSourceFile(object sender, RoutedEventArgs args) {
            if (OS.IsWasm()) return;
            var grid = this.FindControl<DataGrid>("OtoGrid");
            var oto = grid?.SelectedItem as UOto;
            if (oto == null) {
                return;
            }
            try {
        ...
        void RegenFrq(object sender, RoutedEventArgs args) {
            if (OS.IsWasm()) {
                MessageBox.Show(this, "Feature not supported on Web.", "Warning", MessageBox.MessageBoxButtons.Ok);
                return;
            }
            var grid = this.FindControl<DataGrid>("OtoGrid");
            if (grid != null &&
                ...
                string[] files = grid.SelectedItems
                    .Cast<UOto>()
                    .Select(oto => oto.File)
                    .ToHashSet()
                    .ToArray();
        ...
        void DrawOto(UOto? oto) {
            var otoPlot = this.FindControl<OtoPlot>("OtoPlot");
            if (otoPlot == null) return;
            if (oto == null) {
                wavPath = null;
                wav = null;
        ...
        void OnKeyDown(object sender, KeyEventArgs args) {
            // Basic text box check
            if (args.Handled || editingCell) {
                return;
            }
            // Focus check for text box
            var focusMan = TopLevel.GetTopLevel(this)?.FocusManager;
            if (focusMan != null && focusMan.GetFocusedElement() is TextBox) {
                return;
            }

            var viewModel = DataContext as SingersViewModel;
            var otoPlot = this.FindControl<OtoPlot>("OtoPlot");
            var otoGrid = this.FindControl<DataGrid>("OtoGrid");
