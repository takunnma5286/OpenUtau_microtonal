...
//-----------------------------------------------------------------------------
static void SetParametersForGetWindowedWaveform(
    int half_window_length, int x_length, double current_position, int fs,
    double current_f0, int window_type, double window_length_ratio,
...
static void GetWindowedWaveform(const double *x, int x_length, int fs,
                                ...
                                int window_type, double window_length_ratio,
                                double *waveform) {
  ...
  SetParametersForGetWindowedWaveform(
      half_window_length, x_length, current_position, fs, current_f0,
      window_type, window_length_ratio, base_index, safe_index, window);
...
static void GetCentroid(const double *x, int x_length, int fs,
                        double current_f0, int fft_size,
  ...
  GetWindowedWaveform(x, x_length, fs, current_f0, current_position,
                      world::kBlackman, 4.0, forward_real_fft->waveform);
  double power = 0.0;
...
static void GetSmoothedPowerSpectrum(const double *x, int x_length, int fs,
  ...
  for (int i = 0; i < fft_size; ++i)
    forward_real_fft->waveform[i] = 0.0;
  GetWindowedWaveform(x, x_length, fs, current_f0, current_position,
                      world::kHanning, 4.0, forward_real_fft->waveform);
...
static void GetStaticGroupDelay(const double *static_centroid,
                                const double *smoothed_power_spectrum, int fs,
                                double f0, int fft_size,
...
static void GetCoarseAperiodicity(const double *static_group_delay, int fs,
                                  int fft_size, int number_of_aperiodicities,
...
static double D4CLoveTrainSub(const double *x, int fs, int x_length,
                              double current_f0, double current_position,
                              int f0_length, int fft_size, int boundary0,
...
static void D4CLoveTrain(const double *x, int fs, int x_length,
  ...
  int fft_size = static_cast<int>(pow(
      2.0,
      1.0 + static_cast<int>(log(3.0 * fs / lowest_f0 + 1) / world::kLog2)));
  ...
  for (int i = 0; i < f0_length; ++i) {
    ...
    aperiodicity0[i] =
        D4CLoveTrainSub(x, fs, x_length, MyMaxDouble(f0[i], lowest_f0),
                        temporal_positions[i], f0_length, fft_size, boundary0,
                        boundary1, boundary2, &forward_real_fft);
  }
...
static void D4CGeneralBody(const double *x, int x_length, int fs,
                           double current_f0, int fft_size,
                           double current_position,
                           int number_of_aperiodicities, const double *window,
                           int window_length,
                           const ForwardRealFFT *forward_real_fft,
                           double *coarse_aperiodicity) {
  ...
  for (int i = 0; i < fft_size / 2 + 1; ++i) {
    smoothed_power_spectrum[i] =
        MyMaxDouble(world::kMySafeGuardMinimum, smoothed_power_spectrum[i]);
  }
  GetStaticGroupDelay(static_centroid, smoothed_power_spectrum, fs, current_f0,
                      fft_size, static_group_delay);

  GetCoarseAperiodicity(static_group_delay, fs, fft_size,
                        number_of_aperiodicities, window, window_length,
                        forward_real_fft, coarse_aperiodicity);
...
static void GetAperiodicity(const double *coarse_frequency_axis,
                            const double *coarse_aperiodicity,
                            int number_of_aperiodicities,
                            const double *frequency_axis, int fft_size,
                            double *aperiodicity) {
...
void D4C(const double *x, int x_length, int fs,
  ...
  int fft_size_d4c = static_cast<int>(
      pow(2.0, 1.0 + static_cast<int>(log(4.0 * fs / world::kFloorF0D4C + 1) /
                                      world::kLog2)));

  ForwardRealFFT forward_real_fft = {0};
  InitializeForwardRealFFT(fft_size_d4c, &forward_real_fft);

  int number_of_aperiodicities = static_cast<int>(
  ...
  for (int i = 0; i < f0_length; ++i) {
    ...
    D4CGeneralBody(x, x_length, fs, MyMaxDouble(world::kFloorF0D4C, f0[i]),
                   fft_size_d4c, temporal_positions[i],
                   number_of_aperiodicities, window, window_length,
                   &forward_real_fft, &coarse_aperiodicity[1]);
    ...
    GetAperiodicity(coarse_frequency_axis, coarse_aperiodicity,
                    number_of_aperiodicities, frequency_axis, fft_size,
                    aperiodicity[i]);
  }
